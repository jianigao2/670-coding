...
to-report are-there-wolves-near-me
  let wolves-near-me
  (list
    (count wolves-on patch-at-heading-and-distance 0 1)
    (count wolves-on patch-at-heading-and-distance -90 1)
    (count wolves-on patch-at-heading-and-distance -180 1)
    (count wolves-on patch-at-heading-and-distance 90 1)
  )
  report wolves-near-me
end

to move-sheep  ; sheep procedure
               ;; SHEEP SCARED OF WOLVES
               ;; find out which directions have most wolves
               ;; chose a direction away from that

  ;;; STEP ONE: where are the wolves near by?
  let near-wolves are-there-wolves-near-me

  ifelse ( sheep-scared-of-wolves)
  [;; sheep are scared of wolves, use new code


    ;; STEP TWO: reacting to nearby wolves
    ifelse ( max near-wolves > 0 )
    [ ;; there are some wolves near me! be careful!
      ifelse (item 0 near-wolves = max near-wolves)
      [ ;; they're in front of me! turn around!
        rt 180
        fd 3
      ]
      [ ;; they are not in front of me. run away!
        fd 3
      ]
    ]
    [ ;; there are no wolves near me, act normally.
      rt random 50
      lt random 50
      fd 1
    ]
  ]
  [ ;; sheep are not scared of wolves, act normally
    rt random 50
    lt random 50
    fd 1
  ]
...
to-report sheep-scared-of-wolves-plotter
  let max-val initial-number-sheep + initial-number-wolves
  ifelse sheep-scared-of-wolves [ report max-val ] [ report 0]
end
